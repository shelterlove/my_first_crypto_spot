...
| --- | --- | --- |
| `dev` | `20180630-20241231` | Strategy design and trade-level diagnostics using all available clean-start non-validation history |
...
The local BTC/ETH/BNB daily data has a common usable start after BNB begins on
`2017-11-06`. The dev split starts on `2018-06-30`, after all three pairs have
enough daily startup candles for the Freqtrade adapter. If older common history
is added later, extend `dev` backward while keeping `validation` and `holdout`
untouched.

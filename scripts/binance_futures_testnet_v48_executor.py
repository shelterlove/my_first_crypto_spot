#!/usr/bin/env python3
"""Execute V4.8 ETH/BNB targets on Binance USD-M Futures Testnet.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="results/binance_futures_testnet_v48")
...
def main() -> None:
    ...
    wallet_balance = futures_wallet_balance(account, args.quote_asset)
    if wallet_balance <= 0:
        raise SystemExit(f"No positive {args.quote_asset} wallet balance available in Futures Testnet account.")
    deploy_equity = wallet_balance * args.sleeve_fraction
    ...
    report = {
        ...
        "symbols": symbols,
        "wallet_balance_usdt": str(wallet_balance),
...
def validate_deployment_symbols(symbols: list[str]) -> None:
    ...
    actual = set(symbols)
    if actual != expected:
        raise SystemExit(f"V4.8 futures deployment symbols must be exactly {sorted(expected)}, got {symbols}.")
...
def reject_short_target_positions(rows: list[dict[str, Any]], wanted_symbols: set[str]) -> None:
    ...
    if shorts:
        raise SystemExit(f"Short target positions detected. Close them before running V4.8 long-only executor: {', '.join(shorts)}")
...
def plan_futures_order(
    *,
    ...
    if position.position_amt < 0:
        raise SystemExit(f"{api_symbol} has a short position. This V4.8 executor is long-only.")
    ...
    target_gross = min(Decimal(str(max(0.0, raw_target_gross))), target_gross_cap)
    ...
    return PlannedFuturesOrder(
        ...
        reason=(
            f"{strategy_name}_futures-rebalance"
...
def futures_wallet_balance(account: dict[str, Any], quote_asset: str) -> Decimal:
    for asset in account.get("assets", []):
        if asset.get("asset") == quote_asset:
            return Decimal(str(asset.get("walletBalance", "0")))
    return Decimal("0")

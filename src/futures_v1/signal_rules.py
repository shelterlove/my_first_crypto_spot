...
from .strategy_types import StrategyContext, StrategyRegime, StrategySignals


class StrategySignalMixin:
    def _build_signals(self, context: StrategyContext, regime: StrategyRegime, episode: dict) -> StrategySignals:
        starter = self._starter_signal(context, regime)
        value_recovery = self._value_recovery(context, regime)
        trend_continuation = self._trend_continuation(context, regime)
        distribution_exhaustion = self._distribution_exhaustion(context, regime)
        recovery_quality_ok = self._recovery_quality_ok(context, regime)
        recovery_signal = self._recovery_signal_from_parts(
            context=context,
            regime=regime,
    ...
    def _accumulation_signal(self, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> bool:
    ...
    def _starter_signal(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        return bool(
    ...
    def _value_recovery(self, context: StrategyContext, regime: StrategyRegime) -> bool:
    ...
    def _trend_continuation(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        latest = context.latest
        price = context.price
    ...
    def _late_trend_continuation_risk(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        latest = context.latest
        ...
        return bool(hot_location and (extended or momentum_dulling or self._distribution_exhaustion(context, regime)))

    def _distribution_exhaustion(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        latest = context.latest
        ...
        rolling_pos = self._value(latest, "rolling_365d_pos")
        roc_20 = self._value(latest, "roc_20")
        atr_rank = self._value(latest, "atr_pct_rank")
        price_vs_ema72 = regime.price_vs_ema72
        return bool(
    ...
    def _recovery_signal_from_parts(
        self,
        *,
        ...
        price_vs_ema72 = regime.price_vs_ema72
        roc_20 = self._value(latest, "roc_20")
        ema24_slope = self._value(latest, "ema24_slope")
        donchian_pos = self._value(latest, "donchian_pos")
        return bool(
    ...
    def _recovery_signal(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        return self._recovery_signal_from_parts(
            context=context,
            regime=regime,
            value_recovery=self._value_recovery(context, regime),
            trend_continuation=self._trend_continuation(context, regime),
    ...
    def _strong_recovery_signal(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        return bool(
            self._recovery_quality_ok(context, regime)
            or self._trend_continuation(context, regime)
            or (self._value_recovery(context, regime) and context.trend_risk <= 1)
        )

    def _recovery_quality_ok(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        latest = context.latest
        atr_rank = self._value(latest, "atr_pct_rank", 0.5)

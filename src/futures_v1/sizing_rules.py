...
from .strategy_types import StrategyContext, StrategyDecisionPlan, StrategyRecoveryPlan, StrategyRegime, StrategySignals, StrategySizing, StrategyTargetPlan


class StrategySizingMixin:
    def _compute_sizing(self, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals, decision: StrategyDecisionPlan) -> StrategySizing:
        ...
        if self._call_count - last_buy < cooldown:
            self._diag["core_cooldown_blocked_count"] += 1
        ...
        if buy_qty * price < self.min_notional:
            self._diag["core_min_notional_blocked_count"] += 1
    ...
    def _hold_recovery_overlay_sizing(
        ...
        if not bool(plan.get("allowed", False)):
            ...
            if blocked not in {"insufficient_sell_to_low_drawdown", "recovery_signal_missing"}:
                self._diag["core_limited_recovery_overlay_blocked_count"] += 1
            return StrategySizing()
        ...
        self._diag["core_limited_recovery_overlay_count"] += 1
    ...
    def _hold_recovery_overlay_plan(
        ...
        if state not in {"DEFENSE_LOCK", "RECOVERY_TEST", "FAILED_RECOVERY_LOCK"}:
            return StrategyRecoveryPlan(blocked_reason="not_overlay_episode")
        if signals.recovery_signal:
            return StrategyRecoveryPlan(blocked_reason="recovery_signal_present")
    ...
    def _structural_exit_max_sell(self, context: StrategyContext, regime: StrategyRegime, episode: dict) -> float:
    ...
    def _sell_block_reason(
        ...
        if not blocked:
            return ""
        self._diag["core_recovery_test_sell_blocked_count"] += 1
        return blocked
    ...
    def _buy_block_reason(
        ...
        if state in {"DEFENSE_LOCK", "RECOVERY_TEST", "FAILED_RECOVERY_LOCK", "STRUCTURAL_BEAR_LOCK"}:
            ...
            if not bool(plan.get("allowed", False)):
                if state in {"DEFENSE_LOCK", "RECOVERY_TEST"}:
                    self._diag["core_defense_recovery_blocked_count"] += 1
        ...
        if state == "DISTRIBUTION_LOCK":
            ...
            if not bool(plan.get("allowed", False)):
                self._diag["core_distribution_reentry_blocked_count"] += 1
        ...
        if state == "FAILED_RECOVERY_LOCK":
            ...
            if signal_count <= 0 and not signals.strong_recovery_signal:
                self._diag["core_failed_recovery_probe_blocked_count"] += 1
    ...
    def _buy_cooldown(self, context: StrategyContext, regime: StrategyRegime, setup: str) -> int:
    ...
    def _max_buy_pct(self, context: StrategyContext, regime: StrategyRegime, episode: dict, setup: str, signals: StrategySignals | None = None) -> float:
    ...
    def _buy_sizing_guard(
        ...
        if base_accumulate_needed:
            guard = self._join_guard(guard, "core_bear_base")
    ...
    def _sell_sizing_guard(
        ...
        guard = self._sizing_guard(context, regime, episode, setup)
        if base_exit_sell:
            guard = self._join_guard(guard, sleeve_guard or "core_bear_base_exit")
        return guard

    def _sizing_guard(self, context: StrategyContext, regime: StrategyRegime, episode: dict, setup: str) -> str:
        parts = [
            f"core_intent_{self._setup_intent_name(setup)}",

"""Base sizing rules for V1 raw decisions."""

from __future__ import annotations

import pandas as pd

from ..strategy_types import StrategyContext, StrategyDecisionPlan, StrategyRegime, StrategySignals, StrategySizing, StrategyTargetPlan


class SizingEngine:
    def compute_sizing(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals, decision: StrategyDecisionPlan) -> StrategySizing:
        ...
        if owner._route_eth_recovery_buy_to_base(context, regime, episode, signals, sizing):
            ...
            from ..strategy_types import StrategySleevePlan
    ...
    def base_compute_sizing(
        ...
        if price <= 0.0 or total_value <= 0.0:
            return StrategySizing(blocked_reason="bad_price_or_value")
        ...
        if current_pct >= target - buy_threshold:
            return StrategySizing(blocked_reason="target_reached")
        ...
        if blocked:
            return StrategySizing(side="buy", setup=setup, blocked_reason=blocked)
        ...
        if owner._call_count - last_buy < cooldown:
            ...
            return StrategySizing(side="buy", setup=setup, blocked_reason="cooldown")
        ...
        if buy_qty <= 1e-12:
            return StrategySizing(side="buy", setup=setup, blocked_reason="zero_quantity")
        if buy_qty * price < owner.min_notional:
            owner._diag["core_min_notional_blocked_count"] += 1
            return StrategySizing(side="buy", setup=setup, blocked_reason="min_notional")
        return StrategySizing(
            side="buy",
            setup=setup,
            quantity=buy_qty,
    ...
    def hold_recovery_overlay_sizing(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> StrategySizing:
        ...
        if not bool(plan.get("allowed", False)):
            ...
            if blocked not in {"insufficient_sell_to_low_drawdown", "recovery_signal_missing"}:
                owner._diag["core_limited_recovery_overlay_blocked_count"] += 1
            return StrategySizing()
        ...
        if context.current_pct >= target - owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup=setup, blocked_reason="target_reached")
        ...
        owner._diag["core_limited_recovery_overlay_count"] += 1
        overlay_guard = "core_limited_recovery_overlay"
    ...
    def sell_sizing(
        ...
        if blocked:
            return StrategySizing(side="sell", setup=setup, blocked_reason=blocked)
        if current_pct <= target + owner.MIN_ADJUST_THRESHOLD:
            return StrategySizing(side="sell", setup=setup, blocked_reason="target_reached")
        ...
        if sell_qty <= 1e-12:
            ...
            return StrategySizing(side="sell", setup=setup, blocked_reason=blocked_reason)
        if sell_qty * price < owner.min_notional:
            owner._diag["core_min_notional_blocked_count"] += 1
            return StrategySizing(side="sell", setup=setup, blocked_reason="min_notional")
    ...
    @staticmethod
    def structural_exit_max_sell(context: StrategyContext, regime: StrategyRegime, episode: dict) -> float:
    ...
    def sell_block_reason(
        ...
        if not blocked:
            return ""
        owner._diag["core_recovery_test_sell_blocked_count"] += 1
        return blocked

    @staticmethod
    def recovery_test_sell_block_reason(owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> str:
        if str(episode.get("state", "NORMAL")) != "RECOVERY_TEST":
            return ""
    ...
    def buy_block_reason(
        ...
        if state in {"DEFENSE_LOCK", "RECOVERY_TEST", "FAILED_RECOVERY_LOCK", "STRUCTURAL_BEAR_LOCK"}:
            ...
            if not bool(plan.get("allowed", False)):
                if state in {"DEFENSE_LOCK", "RECOVERY_TEST"}:
                    owner._diag["core_defense_recovery_blocked_count"] += 1
        ...
        if state == "DISTRIBUTION_LOCK":
            ...
            if not bool(plan.get("allowed", False)):
                owner._diag["core_distribution_reentry_blocked_count"] += 1
        ...
        if state == "FAILED_RECOVERY_LOCK":
            ...
            if signal_count <= 0 and not signals.strong_recovery_signal:
                owner._diag["core_failed_recovery_probe_blocked_count"] += 1
    ...
    @staticmethod
    def buy_cooldown(owner, context: StrategyContext, regime: StrategyRegime, setup: str) -> int:
    ...
    def max_buy_pct(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, setup: str, signals: StrategySignals | None = None) -> float:
    ...
    def buy_sizing_guard(
        ...
        if base_accumulate_needed:
            guard = owner._join_guard(guard, "core_bear_base")
    ...
    @staticmethod
    def sell_sizing_guard(owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals, setup: str, base_exit_sell: bool = False, sleeve_guard: str = "") -> str:
        guard = owner._sizing_guard(context, regime, episode, setup)
        if base_exit_sell:
            guard = owner._join_guard(guard, sleeve_guard or "core_bear_base_exit")
        return guard

    @staticmethod
    def sizing_guard(owner, context: StrategyContext, regime: StrategyRegime, episode: dict, setup: str) -> str:
        parts = [
            f"core_intent_{owner._setup_intent_name(setup)}",
            f"core_regime_{regime.regime.lower()}",
        ]
        state = str(episode.get("state", "NORMAL"))
        if state != "NORMAL":
            parts.append(f"core_episode_{state.lower()}")
        if setup == "recovery-probe-buy":
            parts.append("core_staged_recovery")
        if state in {"DEFENSE_LOCK", "RECOVERY_TEST", "FAILED_RECOVERY_LOCK", "STRUCTURAL_BEAR_LOCK"}:
            sell_price = float(episode.get("sell_price", 0.0) or 0.0)

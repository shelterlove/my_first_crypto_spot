"""Risk-cycle episode and recovery-credit ledger for V1."""
...
from ..strategy_types import StrategyContext, StrategyRegime, StrategySignals, StrategySizing


class EpisodeOwner(Protocol):
    ...
    _current_context_by_symbol: dict[str, StrategyContext]

    def _recovery_quality_ok(self, context: StrategyContext, regime: StrategyRegime) -> bool:
        ...

    def _failed_recovery(self, context: StrategyContext, episode: dict) -> bool:
        ...
...
class EpisodeEngine:
    ...
    def update_episode(self, owner: EpisodeOwner, context: StrategyContext, regime: StrategyRegime) -> dict:
    ...
    def close_episode(self, owner: EpisodeOwner, symbol: str, reason: str) -> None:
        ...
        if reason == "recovery_confirmed" and not reconciled:
            ...
            owner._diag["core_recovery_signal_confirmed_budget_open_count"] = (
                owner._diag.get("core_recovery_signal_confirmed_budget_open_count", 0) + 1
            )
        ...
        owner._diag["core_episode_closed_count"] += 1

    def append_episode_sell_leg(self, owner, context: StrategyContext, episode: dict, sizing: StrategySizing) -> None:
    ...
    def open_recovery_credit(self, owner: EpisodeOwner, symbol: str, episode: dict, open_budget: float, reason: str) -> None:
    ...
    def decay_recovery_credit(self, owner: EpisodeOwner, context: StrategyContext) -> None:
    ...
    @staticmethod
    def recovery_credit_total(owner: EpisodeOwner) -> float:
        return sum(float(item.get("remaining", 0.0) or 0.0) for item in owner._recovery_credit_ledger.values())

    @staticmethod
    def recovery_credit_portfolio_cap(owner: EpisodeOwner) -> float:

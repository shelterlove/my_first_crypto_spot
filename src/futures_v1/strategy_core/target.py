"""Target-vector construction for V1 raw decisions."""
...
class TargetEngine:
    ...
    def compose_target_from_plan(self, owner: TargetOwner, context: StrategyContext, plan: StrategyTargetPlan) -> float:
    ...
    def build_target_plan(
        ...
        if intent == "EXIT":
            ...
            if tactical_target <= owner.RECOVERY_MIN_STEP and bear_base_floor > 0.0:
                owner._diag["core_bear_base_floor_sell_protected_count"] += 1
    ...
    def main_tactical_target(self, owner: TargetOwner, context: StrategyContext, regime: StrategyRegime) -> float:
    ...
    @staticmethod
    def combine_base_tactical_target(owner: TargetOwner, base_target: float, tactical_target: float) -> float:
    ...
    @staticmethod
    def vol_multiplier(context: StrategyContext, regime: StrategyRegime) -> float:
    ...
    @staticmethod
    def apply_core_floor(owner: TargetOwner, symbol: str, tactical_target: float) -> float:

...
from ..strategy_types import StrategyContext, StrategyDecisionPlan, StrategyRecoveryPlan, StrategyRegime, StrategySignals, StrategySizing, StrategySleevePlan
...
class RecoveryEngine:
    ...
    def episode_recovery_plan(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> StrategyRecoveryPlan:
        return self.base_episode_recovery_plan(owner, context, regime, episode, signals)

    def base_episode_recovery_plan(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> StrategyRecoveryPlan:
    ...
    @staticmethod
    def episode_reentry_plan(owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> StrategyRecoveryPlan:
        sell_price = float(episode.get("sell_price", 0.0) or 0.0)
        if sell_price <= 0.0:
            return StrategyRecoveryPlan(blocked_reason="missing_sell_anchor")
        age = owner._call_count - int(episode.get("start_call", owner._call_count))
    ...
    def episode_recovery_target(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals, base_target: float) -> float:
    ...
    def episode_recovery_max_buy(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, setup: str, signals: StrategySignals | None = None) -> float:
        if signals is None:
    ...
    def record_episode_recovery_buy(self, owner, context: StrategyContext, episode: dict, action: Action) -> None:
        buy_pct = action.quantity * action.price / context.total_value if context.total_value > 0.0 else 0.0
        owner._diag["core_recovery_path_buy_count"] += 1
    ...
    @staticmethod
    def is_bear_base_buy(decision: StrategyDecisionPlan, sizing: StrategySizing) -> bool:
    ...
    def recovery_permission(self, owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals, low_drop: float) -> StrategyRecoveryPlan:
        return self.base_recovery_permission(context, regime, signals, low_drop)

    @staticmethod
    def base_recovery_permission(context: StrategyContext, regime: StrategyRegime, signals: StrategySignals, low_drop: float) -> StrategyRecoveryPlan:
    ...
    def staged_recovery_plan(self, owner, *, context: StrategyContext, regime: StrategyRegime, episode: dict, age: int, current_drop: float, low_drop: float, remaining: float, lowest: float) -> StrategyRecoveryPlan:
    ...
    @staticmethod
    def deep_base_recovery_plan(owner, *, context: StrategyContext, regime: StrategyRegime, episode: dict, age: int, current_drop: float, low_drop: float, remaining: float) -> StrategyRecoveryPlan:
        ...
        if low_drop >= 0.45 and age >= owner.DEEP_BASE_MIN_CALLS + 45:
            ...
            fraction = 0.24
        return StrategyRecoveryPlan(allowed=True, fraction=fraction, guard="core_deep_base_recovery")

    @staticmethod
    def post_crash_recoil_plan(owner, *, context: StrategyContext, regime: StrategyRegime, episode: dict, age: int, current_drop: float, low_drop: float, remaining: float, lowest: float) -> StrategyRecoveryPlan:
        ...
        if low_drop >= 0.35 and age >= 30:
            ...
            fraction = 0.18
        return StrategyRecoveryPlan(allowed=True, fraction=fraction, guard="core_post_crash_recoil")

    @staticmethod
    def structural_recovery_ready(context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> bool:
        return bool(
    ...
    @staticmethod
    def base_recovery_established(owner, context: StrategyContext) -> bool:
    ...
    @staticmethod
    def base_recovery_stabilized(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> bool:
    ...
    @staticmethod
    def base_recovery_target(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> float:
    ...
    def base_recovery_should_accumulate(self, owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> bool:
    ...
    def base_led_recovery_base_sizing(self, owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals, decision: StrategyDecisionPlan, primary: StrategySizing) -> StrategySizing:
        ...
        if buy_qty <= 1e-12:
            return StrategySizing(side="buy", setup="base-led-recovery-buy", blocked_reason="zero_quantity")
        if buy_qty * context.price < owner.min_notional:
            owner._diag["core_min_notional_blocked_count"] += 1
            return StrategySizing(side="buy", setup="base-led-recovery-buy", blocked_reason="min_notional")
    ...
    @staticmethod
    def base_led_recovery_allowed(owner, context: StrategyContext, regime: StrategyRegime, episode: dict, signals: StrategySignals) -> bool:
        if not hasattr(owner, "BASE_LED_RECOVERY_TARGET"):
            return False
        if context.symbol == "BTC/USDT":
            return False

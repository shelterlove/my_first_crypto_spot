"""Accounting boundary for V1 sleeve records."""
...
class AccountingEngine:
    ...
    @staticmethod
    def sync_base_from_sources(owner, context: StrategyContext) -> None:
    ...
    def add_base_source(self, owner, context: StrategyContext, action: Action, source: str, layer: int = 1) -> None:
        ledger = owner._base_ledger_by_symbol.setdefault(context.symbol, owner._new_base_ledger())
        sources = ledger.setdefault("source_ledger", {})
    ...
    @staticmethod
    def source_quantity(owner, context: StrategyContext, source: str) -> float:
    ...
    def record_protected_floor_base_buy(self, owner, context: StrategyContext, action: Action) -> None:
        ledger = owner._base_ledger_by_symbol.setdefault(context.symbol, owner._new_base_ledger())
    ...
    def record_bear_base_buy(self, owner, context: StrategyContext, action: Action) -> None:
        ...
        if layer == 1:
            owner._diag["core_bear_base_layer1_buy_count"] += 1
        elif layer == 2:
            owner._diag["core_bear_base_layer2_buy_count"] += 1

        self.add_base_source(owner, context, action, "strategic_base", layer=layer)

    def record_base_led_recovery_buy(self, owner, context: StrategyContext, action: Action) -> None:
        self.add_base_source(owner, context, action, "base_led_recovery", layer=2)

    def record_protected_floor_base_exit(self, owner, context: StrategyContext, action: Action) -> None:
        ledger = owner._base_ledger_by_symbol.get(context.symbol)
        if not ledger:
            return
    ...
    def record_base_exit_sell(self, owner, context: StrategyContext, action: Action) -> None:
        ledger = owner._base_ledger_by_symbol.get(context.symbol)
        if ledger and "source_ledger" in ledger:
            ...
            if consumed > 1e-12:
                owner._diag["core_bear_base_exit_count"] += 1
            return
        ...
        owner._diag["core_bear_base_exit_count"] += 1
        if layer == 1:
            owner._diag["core_bear_base_layer1_exit_count"] += 1
        elif layer == 2:
            owner._diag["core_bear_base_layer2_exit_count"] += 1

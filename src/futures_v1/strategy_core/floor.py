"""Protected low-floor and lifecycle base helpers for V1."""
...
from ..strategy_types import StrategyContext, StrategyDecisionPlan, StrategyRegime, StrategySignals, StrategySizing, StrategySleevePlan
...
class FloorEngine:
    def opportunity_floor_sizing(
        ...
        if target <= 0.0 or context.current_pct >= target - owner.RECOVERY_MIN_STEP:
            return StrategySizing(blocked_reason="opportunity_floor_filled")
        ...
        if not self.opportunity_floor_stabilizing(owner, context, regime, signals):
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="opportunity_floor_not_stabilizing")
        ...
        if owner._call_count - last_buy < owner.OPPORTUNITY_FLOOR_COOLDOWN_CALLS:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="opportunity_floor_cooldown")
        ...
        if buy_qty <= 1e-12:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="zero_quantity")
        if buy_qty * context.price < owner.min_notional:
            owner._diag["core_min_notional_blocked_count"] += 1
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="min_notional")
    ...
    @staticmethod
    def opportunity_floor_low_location(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
    ...
    @staticmethod
    def opportunity_floor_stabilizing(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> bool:
    ...
    @staticmethod
    def protected_floor_quantity(owner, context: StrategyContext) -> float:
    ...
    @staticmethod
    def protected_floor_exit_allowed(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        ledger = owner._protected_floor_ledger_by_symbol.get(context.symbol)
    ...
    @staticmethod
    def record_protected_floor_buy(owner, context: StrategyContext, regime: StrategyRegime, action: Action) -> None:
    ...
    def consume_protected_floor_for_sell(self, owner, context: StrategyContext, regime: StrategyRegime, action: Action) -> None:
    ...
    @staticmethod
    def protected_floor_exit_event(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals, profit: float) -> bool:
    ...
    @staticmethod
    def protected_floor_new_release_event(owner, context: StrategyContext, ledger: dict) -> bool:
    ...
    def lifecycle_low_base_sizing(
        ...
        if context.current_pct > cap + owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_total_exposure_filled")
        ...
        if not self.lifecycle_low_base_stabilizing(owner, context, regime, signals):
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_not_stabilizing")
        if signals.distribution_exhaustion:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_distribution")
        ...
        if source_pct >= cap - owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_cap_filled")
        ...
        if portfolio_room < owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_portfolio_cap")
        exposure_room = max(0.0, owner.TOTAL_EXPOSURE_CAP - float(context.current_pct))
        if exposure_room < owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="total_exposure_cap")
        ...
        if owner._call_count - last_buy < owner.LIFECYCLE_LOW_BASE_COOLDOWN_CALLS:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_cooldown")
        gap = min(max(0.0, cap - source_pct), portfolio_room, exposure_room)
        buy_pct = min(gap, owner.LIFECYCLE_LOW_BASE_MAX_BUY_PCT)
        if buy_pct < owner.RECOVERY_MIN_STEP:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="lifecycle_low_base_step_too_small")
        buy_qty = context.total_value * buy_pct / context.price if context.price > 0.0 else 0.0
        if buy_qty <= 1e-12:
            return StrategySizing(side="buy", setup="opportunity-floor-buy", blocked_reason="zero_quantity")
        if buy_qty * context.price < owner.min_notional:
        ...
        decision.primary_sleeve = StrategySleevePlan(
            sleeve="lifecycle-low-base",
            side="buy",
            setup="opportunity-floor-buy",
            target=min(owner.TOTAL_EXPOSURE_CAP, context.current_pct + buy_pct),
            guard=guard,
            priority=86,
            allowed=True,
        )
    ...
    def lifecycle_protected_base_exit_sizing(
        ...
        if owner._call_count - entry_call < owner.LIFECYCLE_BASE_EXIT_MIN_HOLD_CALLS:
            return StrategySizing(side="sell", setup="protected-floor-exit", blocked_reason="lifecycle_base_exit_min_hold")
        profit = context.price / avg_entry - 1.0
        ...
        sell_qty = source_qty
        if sell_qty <= 1e-12:
            return StrategySizing(side="sell", setup="protected-floor-exit", blocked_reason="zero_quantity")
        notional = sell_qty * context.price
        if notional < owner.min_notional:
            owner._diag["core_min_notional_blocked_count"] += 1
            return StrategySizing(side="sell", setup="protected-floor-exit", blocked_reason="min_notional")
    ...
    @staticmethod
    def lifecycle_base_exit_high_location(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        rolling_pos = owner._value(context.latest, "rolling_365d_pos", 0.5)
        donchian_pos = owner._value(context.latest, "donchian_pos", 0.5)
        return bool(
    ...
    @staticmethod
    def lifecycle_low_base_entry_location(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        rolling_pos = owner._value(context.latest, "rolling_365d_pos", 0.5)
        donchian_pos = owner._value(context.latest, "donchian_pos", 0.5)
        deep_discount = bool(
    ...
    @staticmethod
    def lifecycle_low_base_stabilizing(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> bool:
        if context.risk_score >= 5:
            return False
        recent = context.df.tail(20)
        if recent.empty or "close" not in recent.columns:
    ...
    @staticmethod
    def lifecycle_protected_floor_portfolio_pct(owner, context: StrategyContext) -> float:
        total = 0.0

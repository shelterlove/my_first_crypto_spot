"""Bear-base budget, location, and exit helpers for V1."""
...
class BearBaseEngine:
    @staticmethod
    def base_quantity(owner, context: StrategyContext) -> float:
    ...
    def main_quantity(self, owner, context: StrategyContext) -> float:
        return max(0.0, float(context.pos.quantity) - self.base_quantity(owner, context))

    def base_pct_from_quantity(self, owner, context: StrategyContext, quantity: float) -> float:
    ...
    def refresh_base_ledger_market(self, owner, context: StrategyContext) -> None:
        ledger = owner._base_ledger_by_symbol.get(context.symbol)
    ...
    def record_ledger_sell(self, owner, context: StrategyContext, sizing: StrategySizing) -> None:
    ...
    def bear_base_floor(self, owner, context: StrategyContext) -> float:
        return self.base_pct_from_quantity(owner, context, self.base_quantity(owner, context))

    def bear_base_target(self, owner, context: StrategyContext, regime: StrategyRegime) -> float:
        ...
        if self.bear_base_path_accelerating(owner, context, regime):
            owner._diag["core_bear_base_accelerating_blocked_count"] += 1
            return 0.0
    ...
    def bear_base_exit_target(self, owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals) -> float:
        ledger = owner._base_ledger_by_symbol.get(context.symbol)
        if not ledger:
            return 0.0
    ...
    @staticmethod
    def bear_base_maturity_score(owner, context: StrategyContext, regime: StrategyRegime, signals: StrategySignals, profit: float) -> int:
        ledger = owner._base_ledger_by_symbol.get(context.symbol, {})
    ...
    @staticmethod
    def bear_base_current_discount(context: StrategyContext, recovery_ledger: dict) -> float:
        avg_sell_price = float(recovery_ledger.get("avg_sell_price", 0.0) or 0.0)
        if avg_sell_price <= 0.0:
            return 0.0
    ...
    @staticmethod
    def bear_base_low_location(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        latest = context.latest
        rolling_pos = owner._value(latest, "rolling_365d_pos", 0.5)
        donchian_pos = owner._value(latest, "donchian_pos", 0.5)
        price_vs_ema168 = regime.price_vs_ema168
    ...
    @staticmethod
    def bear_base_location_depth(owner, context: StrategyContext) -> float:
        rolling_pos = owner._value(context.latest, "rolling_365d_pos", 0.5)
    ...
    @staticmethod
    def bear_base_path_accelerating(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        roc_20 = owner._value(context.latest, "roc_20", 0.0)
        atr_rank = regime.atr_rank
        return bool(
    ...
    @staticmethod
    def bear_base_path_stabilizing(owner, context: StrategyContext, regime: StrategyRegime) -> bool:
        roc_20 = owner._value(context.latest, "roc_20", 0.0)
        atr_rank = regime.atr_rank
        donchian_pos = owner._value(context.latest, "donchian_pos", 0.5)
        return bool(

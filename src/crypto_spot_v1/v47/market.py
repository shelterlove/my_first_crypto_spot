"""Market context and regime construction for V4.7."""
...
from ..v42_types import V42Context, V42Regime


class V47MarketOwner(Protocol):
...
class V47MarketEngine:
    """Builds the raw spot market view used by the V4.7 decision pipeline."""

    def build_context(
        self,
        owner: V47MarketOwner,
        ...
        owner._diag["v4_2_context_built_count"] += 1
        context = V42Context(
            symbol=symbol,
            df=df,
            latest=latest,
    ...
    def build_regime(self, owner: V47MarketOwner, context: V42Context) -> V42Regime:
        ...
        owner._diag[f"v4_2_regime_{regime.lower()}_count"] += 1
        return V42Regime(
            regime=regime,
            reason=reason,
    ...
    @staticmethod
    def apply_state_confirmation(owner: V47MarketOwner, symbol: str, raw_state: str) -> str:

"""Event and lifecycle lookup helpers for V4.7."""
...
class V47EventEngine:
    ...
    def _record_event_opportunity_floor_v3(
        ...
        owner._diag["v4_2_action_buy_count"] += 1
    ...
    def _record_event_recovery_credit_soft(
        ...
        self._record_event_v42_execution(owner, context, regime, episode, decision, sizing, action)
    ...
    @staticmethod
    def _record_event_v42_execution(
        owner,
        context: V42Context,
        regime: V42Regime,
        episode: dict,
        ...
        if blocked and blocked != "target_reached":
            owner._diag["v4_2_sizing_blocked_count"] += 1
        ...
        owner._diag[f"v4_2_action_{action.side}_count"] += 1
        setup_key = sizing.setup.replace("-", "_")
        diag_key = f"v4_2_{setup_key}_count"
        ...
        if action.side == "buy":
            ...
            if bear_base_buy:
                owner._diag["v4_2_bear_base_buy_count"] += 1
                owner._record_bear_base_buy(context, action)
            if "v4_2_deep_base_recovery" in str(sizing.guard):
                owner._diag["v4_2_deep_base_recovery_count"] += 1
            if "v4_2_post_crash_recoil" in str(sizing.guard):
                owner._diag["v4_2_post_crash_recoil_count"] += 1
            active = owner._episodes_by_symbol.get(symbol)
        ...
        if action.side == "sell" and (primary_sleeve == "bear-base-exit" or "v4_2_bear_base_exit" in str(sizing.guard)):
        ...
        if action.side == "sell" and sizing.setup in {"defense-sell", "structural-exit-sell", "distribution-sell"}:
            if sizing.setup in {"defense-sell", "structural-exit-sell"}:
                ...
                if owner._base_quantity(context) > 1e-12:
                    owner._diag["v4_2_would_force_base_exit_count"] += 1
            owner._start_episode(context, sizing)
        if hasattr(owner, "_record_sleeve_accounting"):
            owner._record_sleeve_accounting(context, decision, sizing, action)

    @staticmethod
    def action_sleeve(decision: V42DecisionPlan, sizing: V42Sizing) -> str:
        ...
        if primary in {"bear-base", "bear-base-exit"}:
            return "base"
        if sizing.setup == "bear-base-exit" or "v4_2_bear_base" in str(sizing.guard):
            return "base"
        return "main"

"""Raw decision orchestration for V4.7.

This module owns the V4.7 decision pipeline order.  The individual decision
...
class V47RawDecisionOwner(Protocol):
    _call_count: int
...
class V47RawDecisionEngine:
    """Runs the raw spot decision pipeline for one event-driven backtest step."""

    def compute_actions(
        self,
        owner: V47RawDecisionOwner,
        candles_by_symbol: dict[str, pd.DataFrame],
        portfolio: PortfolioState,
        current_prices: dict[str, float],
    ) -> list[Action]:

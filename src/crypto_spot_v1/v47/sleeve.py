"""Sleeve candidate construction and arbitration for V4.7."""

from __future__ import annotations

from typing import Protocol

from ..v42_types import V42Context, V42RecoveryPlan, V42Regime, V42Signals, V42SleevePlan, V42TargetPlan


class V47SleeveOwner(Protocol):
...
class V47SleeveEngine:
    """Builds candidate orders and selects the primary raw sleeve."""

    def build_sleeve_plans(
        self,
        owner: V47SleeveOwner,
        context: V42Context,
        regime: V42Regime,
        episode: dict,
        signals: V42Signals,
        intent: str,
        target_plan: V42TargetPlan,
        target: float,
    ) -> tuple[V42SleevePlan, ...]:
        plans: list[V42SleevePlan] = []
        if target_plan.base_exit_distribute:
            ...
            plans.append(V42SleevePlan(
                sleeve="bear-base-exit",
                side="sell",
                setup=base_exit_setup,
                target=target,
                guard="v4_2_bear_base_exit",
                priority=100,
                allowed=True,
            ))
        ...
        if intent == "HOLD":
            ...
            if bool(overlay_plan.get("allowed", False)):
                plans.append(V42SleevePlan(
                    ...
                    guard="v4_2_limited_recovery_overlay",
                    priority=40,
                    allowed=True,
                ))
        if not plans:
            plans.append(V42SleevePlan(sleeve="main", target=target, allowed=False, blocked_reason="hold"))
        return tuple(plans)

    @staticmethod
    def select_primary_sleeve(sleeve_plans: tuple[V42SleevePlan, ...]) -> V42SleevePlan:
        allowed = [plan for plan in sleeve_plans if plan.allowed]
        if not allowed:
            return sleeve_plans[0] if sleeve_plans else V42SleevePlan(blocked_reason="no_sleeve_plan")
    ...
    def buy_setup_from_plan(
        ...
        episode: dict,
        signals: V42Signals,
        target_plan: V42TargetPlan | None,
    ) -> str:

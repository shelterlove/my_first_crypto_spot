...
from .v42_types import V42Context, V42RecoveryPlan, V42Regime, V42Signals


class V42RecoveryMixin:
    ...
    def _episode_reentry_plan(self, context: V42Context, regime: V42Regime, episode: dict, signals: V42Signals) -> V42RecoveryPlan:
        sell_price = float(episode.get("sell_price", 0.0) or 0.0)
        if sell_price <= 0.0:
            return V42RecoveryPlan(blocked_reason="missing_sell_anchor")
        age = self._call_count - int(episode.get("start_call", self._call_count))
    ...
    def _record_episode_recovery_buy(self, context: V42Context, episode: dict, action: Action) -> None:
        buy_pct = action.quantity * action.price / context.total_value if context.total_value > 0.0 else 0.0
        self._diag["v4_2_recovery_path_buy_count"] += 1
    ...
    def _is_bear_base_buy(self, decision: V42DecisionPlan, sizing: V42Sizing) -> bool:
    ...
    def _staged_recovery_ready(self, context: V42Context, regime: V42Regime, episode: dict) -> bool:
        sell_price = float(episode.get("sell_price", 0.0) or 0.0)
        if sell_price <= 0.0:
            return False
        age = self._call_count - int(episode.get("start_call", self._call_count))
    ...
    def _post_crash_recoil_plan(
        ...
        *,
        ...
        return V42RecoveryPlan(
            allowed=True,
            fraction=fraction,
            guard="v4_2_post_crash_recoil",
        )

    def _structural_recovery_ready(self, context: V42Context, regime: V42Regime, signals: V42Signals) -> bool:
        return bool(

...
from .v42_types import V42Context, V42DecisionPlan, V42Regime, V42Signals


class V42DecisionMixin:
    def _choose_intent(self, context: V42Context, regime: V42Regime, episode: dict, signals: V42Signals) -> str:
        ...
        if bool(getattr(self, "ENABLE_HOLD_ONLY_BASE_INTENT", False)) and base_accumulate_needed:
            if intent == "HOLD" and self._base_intent_hard_gate_allows(context, regime):
                intent = "ACCUMULATE"
                self._diag["v4_2_base_intent_accumulate_count"] += 1
            elif intent == "DEFEND":
                self._diag["v4_2_base_deferred_by_main_defense_count"] += 1
                ...
                self._diag["v4_2_base_intent_deferred_count"] += 1
                self._record_base_deferred_candidate(
                    context,
                    regime,
                    main_intent=intent,
        ...
        self._diag[f"v4_2_intent_{intent.lower()}_count"] += 1
        return intent
    ...
    def _base_intent_hard_gate_allows(self, context: V42Context, regime: V42Regime) -> bool:
        return bool(
    ...
    def _annotate_main_base_intent(self, plan: V42DecisionPlan, context: V42Context) -> None:

...
class V42TargetMixin:
    def _compose_target_from_plan(self, context: V42Context, plan: V42TargetPlan) -> float:
    ...
    def _build_target_plan(
        ...
        if intent == "EXIT":
            ...
            if tactical_target <= self.RECOVERY_MIN_STEP and bear_base_floor > 0.0:
                self._diag["v4_2_bear_base_floor_sell_protected_count"] += 1
    ...
    def _main_tactical_target(self, context: V42Context, regime: V42Regime) -> float:
    ...
    def _vol_multiplier(self, context: V42Context, regime: V42Regime) -> float:

...
class V42MarketMixin:
    def _build_context(self, candles_by_symbol, portfolio, current_prices) -> V42Context | None:
        ...
        self._diag["v4_2_context_built_count"] += 1
        context = V42Context(
            symbol=symbol,
            df=df,
            latest=latest,
    ...
    def _build_regime(self, context: V42Context) -> V42Regime:
        ...
        self._diag[f"v4_2_regime_{regime.lower()}_count"] += 1
        return V42Regime(
            regime=regime,
            reason=reason,

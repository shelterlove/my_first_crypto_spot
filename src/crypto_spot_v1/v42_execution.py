...
class V42ExecutionMixin:
    ...
    def _record_event(self, context: V42Context, regime: V42Regime, episode: dict, decision: V42DecisionPlan, sizing: V42Sizing, action: Action | None) -> None:
        symbol = context.symbol
        blocked = sizing.blocked_reason
        if blocked and blocked != "target_reached":
            self._diag["v4_2_sizing_blocked_count"] += 1
        ...
        self._diag[f"v4_2_action_{action.side}_count"] += 1
        ...
        if action.side == "sell" and sizing.setup in {"defense-sell", "structural-exit-sell", "distribution-sell"}:
            if sizing.setup in {"defense-sell", "structural-exit-sell"}:
                ...
                if self._base_quantity(context) > 1e-12:
                    self._diag["v4_2_would_force_base_exit_count"] += 1

...
class V42BearBaseMixin:
    def _base_quantity(self, context: V42Context) -> float:
    ...
    def _base_pct_from_quantity(self, context: V42Context, quantity: float) -> float:
    ...
    def _refresh_base_ledger_market(self, context: V42Context) -> None:
        ledger = self._base_ledger_by_symbol.get(context.symbol)
    ...
    def _record_ledger_sell(self, context: V42Context, sizing: V42Sizing) -> None:
    ...
    def _record_bear_base_buy(self, context: V42Context, action: Action) -> None:
        ...
        if layer == 1:
            self._diag["v4_2_bear_base_layer1_buy_count"] += 1
        elif layer == 2:
            self._diag["v4_2_bear_base_layer2_buy_count"] += 1
    ...
    def _record_base_exit_sell(self, context: V42Context, action: Action) -> None:
        ...
        if layer == 1:
            self._diag["v4_2_bear_base_layer1_exit_count"] += 1
        elif layer == 2:
            self._diag["v4_2_bear_base_layer2_exit_count"] += 1
    ...
    def _bear_base_target(self, context: V42Context, regime: V42Regime) -> float:
        ...
        if self._bear_base_path_accelerating(context, regime):
            self._diag["v4_2_bear_base_accelerating_blocked_count"] += 1
            return 0.0
    ...
    def _bear_base_maturity_score(self, context: V42Context, regime: V42Regime, signals: V42Signals, profit: float) -> int:
        ledger = self._base_ledger_by_symbol.get(context.symbol, {})
    ...
    @staticmethod
    def _bear_base_layer(context: V42Context, location_depth: float) -> int:
    ...
    @staticmethod
    def _bear_base_current_discount(context: V42Context, recovery_ledger: dict) -> float:
        avg_sell_price = float(recovery_ledger.get("avg_sell_price", 0.0) or 0.0)
        if avg_sell_price <= 0.0:
            return 0.0
        return max(0.0, 1.0 - context.price / avg_sell_price)

    def _bear_base_low_location(self, context: V42Context, regime: V42Regime) -> bool:
        latest = context.latest
        rolling_pos = self._value(latest, "rolling_365d_pos", 0.5)
        donchian_pos = self._value(latest, "donchian_pos", 0.5)
        price_vs_ema168 = regime.price_vs_ema168
    ...
    def _bear_base_location_depth(self, context: V42Context) -> float:
        rolling_pos = self._value(context.latest, "rolling_365d_pos", 0.5)
        if pd.isna(rolling_pos):
            return 0.0
        return max(0.0, 1.0 - float(rolling_pos))

    def _bear_base_path_accelerating(self, context: V42Context, regime: V42Regime) -> bool:
        roc_20 = self._value(context.latest, "roc_20", 0.0)
        atr_rank = regime.atr_rank
        return bool(
    ...
    def _bear_base_path_stabilizing(self, context: V42Context, regime: V42Regime) -> bool:
        roc_20 = self._value(context.latest, "roc_20", 0.0)
        atr_rank = regime.atr_rank
        donchian_pos = self._value(context.latest, "donchian_pos", 0.5)
        return bool(
